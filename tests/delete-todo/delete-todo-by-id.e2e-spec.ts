...
describe(`Delete Todo By ID Test Suite`, () => {
  ...
  it(`must not delete ToDo when no ID provided and return status 404`, async (done) => {
  ...
  it(`must not delete ToDo when ID not in valid format and return status 400`, async (done) => {
    const invalidID = '1';
    const response = await request(app)
      .delete(`/api/todo/id/${invalidID}`)
      .set({ 'x-token-header': jwtToken })
      .send();
    const { status } = response;
  ...
  it(`must not delete ToDo when ID is valid but not in DB and return status 400`, async (done) => {
    const incorrectID = '507f1f77bcf86cd799439011';
    const response = await request(app)
      .delete(`/api/todo/id/${incorrectID}`)
      .set({ 'x-token-header': jwtToken })
      .send();
    const { status } = response;
    expect(status).toBe(400);
  ...
  it(`must delete a To Do item when ID is valid and present in DB and return status 200`, async (done) => {
    const response = await request(app)
